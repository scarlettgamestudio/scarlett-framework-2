import { Utils, Logger } from "scarlett-commons";
import { GLU } from "../webglUtils";
...
export default class Shader {
  ...
  /**
   *
   * @param {WebGLRenderContext} glc Webgl Render Context
   * @param {String} vertexScript
   * @param {String} fragmentScript
   * @param {*} uniforms
   * @param {*} attributes
   */
  constructor(renderContext, vertexScript, fragmentScript, uniforms, attributes) {
    if (!Utils.isObjectAssigned(vertexScript) || !Utils.isObjectAssigned(fragmentScript)) {
      throw new Error("Vertex and Fragment scripts are required to create a shader, discarding..");
    }

    if (!Utils.isObjectAssigned(renderContext)) {
    ...
    this._shaderManager = renderContext.getShaderManager();
    this._gl = renderContext.getContext();
    this._program = null;
    this._vertexScript = vertexScript;
    this._fragmentScript = fragmentScript;
    this._textureCount = 1;
    this._uid = Utils.generateUID();
  ...
  setup() {
    if (this.compile()) {
      if (this._shaderManager) {
        this._shaderManager.useShader(this);
      } else {
  ...
  compile() {
    ...
    if (Utils.isObjectAssigned(program)) {
  ...
  syncUniform(uniform) {
    ...
    switch (uniform.type) {
      ...
      case "tex": {
        ...
        if (!Utils.isObjectAssigned(texture)) {
